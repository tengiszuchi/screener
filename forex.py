...
# Endpoint for getting Forex data using the example API
API_URL = "https://open.er-api.com/v6/latest/USD"  # Using USD as the base currency

def fetch_forex_data():
    """Fetches Forex data from the API and returns it as a list of all currencies."""
    try:
        ...
        if response.status_code == 200:
            ...
            # Extracting data for all currency pairs
            for currency, rate in rates.items():
                pair = f"USD/{currency}"  # Assuming USD is the base currency
                forex_pairs.append([pair, rate])
...
def display_terminal_title(console, start_color, end_color):
    ...
    gradient_colors = generate_gradient_color(start_color, end_color, len("Forex Screener"))
    title_text = Text()

    # Apply gradient to each letter
    for idx, char in enumerate("Forex Screener"):
...
def display_forex_table(forex_data):
    """Displays Forex data in a nicely formatted table."""
    console = Console()
    ...
    gradient_colors = generate_gradient_color(start_color, end_color, len(forex_data))
    ...
    for idx, entry in enumerate(forex_data):
        row_style = Style(color=gradient_colors[idx % len(gradient_colors)], bold=True)
        table.add_row(
            entry[0],  # Currency Pair
            f"{entry[1]:,.4f}",  # Exchange Rate
            style=row_style
        )
...
def main():
    ...
    if forex_data:
        ...
        display_forex_table(forex_data)
    else:
        print("No data found.")
